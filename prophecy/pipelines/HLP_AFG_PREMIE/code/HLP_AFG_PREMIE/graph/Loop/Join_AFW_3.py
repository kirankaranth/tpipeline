    ...
    return met_leeftijdregel\
        ...
        .join(
          ...
          (
            ...
            & (
              (coalesce(col("met_leeftijdregel.LFT_EIND_DTM"), lit("0001-01-01").cast(DateType())) > coalesce(col("afw_premie_zonder.EIND_DTM"), lit("0001-01-01").cast(DateType())))
              & (
                coalesce(col("met_leeftijdregel.LFT_INGANG_DTM"), lit("0001-01-01").cast(DateType()))
                <= coalesce(col("afw_premie_zonder.EIND_DTM"), lit("0001-01-01").cast(DateType()))
              )
            )
          ),
          "left_outer"
        ...
        .join(
          ...
          (
            ...
            & (
              (coalesce(col("met_leeftijdregel.LFT_EIND_DTM"), lit("0001-01-01").cast(DateType())) > coalesce(col("afw_premie_bouwsteen.EIND_DTM"), lit("0001-01-01").cast(DateType())))
              & (
                coalesce(col("met_leeftijdregel.INGANG_VAN_LEEFTIJD"), lit("0001-01-01").cast(DateType()))
                <= coalesce(col("afw_premie_bouwsteen.EIND_DTM"), lit("0001-01-01").cast(DateType()))
              )
            )
          ),
          "left_outer"
